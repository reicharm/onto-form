...
| `headers` | object | — | Statische zusätzliche HTTP-Header |
| `auth` | object | — | Authentifizierungskonfiguration (siehe unten) |

### `fileUpload.auth`-Eigenschaften

| Eigenschaft | Typ | Beschreibung |
|---|---|---|
| `type` | `"none"` \| `"apikey"` \| `"bearer"` \| `"basic"` \| `"oauth2cc"` | Authentifizierungsverfahren |
| `headerName` | string | Header-Name für `apikey` (Standard: `X-API-Key`) |
| `tokenUrl` | string | Token-Endpunkt für `oauth2cc` |
| `scope` | string | OAuth2-Scope (optional, nur `oauth2cc`) |
| `label` | `{de, en}` | Bezeichnung im Anmelde-Formular |

**Sicherheitshinweis:** Die `auth`-Konfiguration deklariert nur das Verfahren (Typ, Header-Name, Token-URL). Zugangsdaten (Passwörter, Tokens, Client-Secrets) werden **niemals** in Konfigurationsdateien gespeichert — `public/` wird vom Webserver ausgeliefert und ist öffentlich einsehbar. Stattdessen gibt der Nutzer die Zugangsdaten beim ersten Upload in einem Formular ein. Sie werden ausschließlich im Arbeitsspeicher gehalten (`UploadAuthStore`, Vue `reactive`) und gehen beim Schließen des Tabs verloren.

### Unterstützte Auth-Verfahren

| Typ | Eingabe | Übertragung |
|---|---|---|
| `none` | – | Keine Authentifizierung |
| `apikey` | Schlüssel | Als `X-API-Key` (oder `headerName`) |
| `bearer` | Token | Als `Authorization: Bearer <token>` |
| `basic` | Benutzername + Passwort | Als `Authorization: Basic <base64>` (RFC 7617) |
| `oauth2cc` | Client-ID + Client-Secret | Token via Client-Credentials-Grant; automatisch gecacht und erneuert |
